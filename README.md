...
Копия шаблона — `.env.example`, рабочий файл `.env` в git не хранится.

| Переменная  | Назначение                                                                       | По умолчанию |
| ----------- | -------------------------------------------------------------------------------- | ------------ |
| `VITE_BASE` | Базовый путь сборки (`base`). Для GitHub Pages в подпапке репозитория — `/DCUT/` | `/`          |

```bash
cp .env.example .env   # задать переменные локально
```

Если `VITE_BASE` не задан, проект собирается в корень (`/`) — удобно для локального `dev` и `preview`. При деплое на GitHub Pages в подпапке переменную задаёт CI (`/DCUT/` в `.github/workflows/deploy.yml`); так же можно собрать вручную:

```bash
VITE_BASE=/DCUT/ npm run build
```
...
Для глубоких ссылок используется фолбэк `404.html`: Vite собирает сайт с базовым путём из переменной `VITE_BASE` (в CI — `/DCUT/`), после сборки `index.html` копируется в `dist/404.html`, и GH Pages отдаёт его для неизвестных маршрутов (React Router рендерит нужную страницу).
